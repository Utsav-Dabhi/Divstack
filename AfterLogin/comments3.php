...
function show_comments($comments, $parent_id = -1) {
    $html = '';
    if ($parent_id != -1) {
        // If the comments are replies sort them by the "created_at" column
...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['name'], $_POST['content'])) {
        ...
        $stmt = $dbh->prepare('INSERT INTO comments (page_id, parent_id, name, content, created_at) VALUES (?,?,?,?,NOW())');
    ...
    $stmt = $dbh->prepare('SELECT * FROM comments WHERE page_id = ? ORDER BY created_at DESC');
    $stmt->execute([ $_GET['page_id'] ]);
    $comments = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the total number of comments
    $stmt = $dbh->prepare('SELECT COUNT(*) AS total_comments FROM comments WHERE page_id = ?');
    $stmt->execute([ $_GET['page_id'] ]);
